...
def main_combined_stress(years: Iterable[int]) -> None:
    ...
    for year in years:
        ...
        waterstress = (ds.soil_depletion*100/TAW).mean("layer").rename("waterstress")
...
def main_yield(years: Iterable[int]) -> None:
    for year in years:
        ...
        csi = xr.open_zarr(f"../data/intermediate/CSI_{year}.zarr", decode_coords="all")\
                .combined_stress
        yield_expectation = (
        ...
        yield_expectation.drop_encoding().to_zarr(f"../data/output/{year}.zarr", mode="a-")
